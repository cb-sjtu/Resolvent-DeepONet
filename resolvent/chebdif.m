...
x = sin(pi*[N-1:-2:1-N]'/(2*(N-1))); % Compute Chebyshev points.%对应的切比雪夫点坐标，等价于cos(j*pi/(N-1))

T = repmat(th/2,1,N);   %重复矩阵，T的维度为N*N
DX = 2*sin(T'+T).*sin(T'-T);          % Trigonometric identity. %用三角函数表达x_i-x_j
DX = [DX(1:n1,:); -flipud(fliplr(DX(1:n2,:)))];   % Flipping trick. %提高计算精度
