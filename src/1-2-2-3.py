...
def network(problem, m,N_points):
    if problem == "ODE":
        ...
        #         tf.keras.layers.InputLayer(input_shape=(173*47*100)),
        #         tf.keras.layers.Reshape((173,47,100)),
...
# andom_uniform
    ...
    if now=="train":
        ...
        for i in range(4): 
            ...
            np.savetxt(dataframe+'/real-Euukc_new_6_train_'+str(i)+'.dat',result)
            #np.savetxt(dataframe+'/loss2'+str(i)+'_branch_w100.dat',branch_out[i])
            with open(dataframe+'/real-Euukc_new_6_train_'+str(i)+'.dat', 'r') as file :
                filedata = file.read()
        ...
        # 创建新的行
            ...
            with open(dataframe+'/real-Euukc_new_6_train_'+str(i)+'.dat', 'w') as file:
                file.write(filedata)

        #积分
        label_train=np.transpose(label_train).reshape((-1,173,200))
        kzs_train=kzss[[0,1,3,4]].reshape((-1,173,1))
        ...
        kzs_test=kzss[[2]].reshape((-1,173,1))
