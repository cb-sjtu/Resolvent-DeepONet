...
def save_results(dataframe, real_2d, label, uum, error, yy, dkxs_s,kzs_s, mode="train"):
    ...
    """
    # 处理2D能谱结果
    label = np.reshape(label, (-1, 176 * 200)).T
    uum = np.reshape(uum, (-1, 176 * 200)).T
    if error is None:
        error = np.abs(label - uum)

    for i in range(label.shape[1]):
        result = np.hstack((
            real_2d[i].reshape((200 * 176, 2)),
            label[:200 * 176, i].reshape((200 * 176, 1)),
            uum[:200 * 176, i].reshape((200 * 176, 1)),
            error[:200 * 176, i].reshape((200 * 176, 1))
        ))
    ...
    # 处理1D积分结果
    label = np.reshape(label.T, (-1, 176, 200))
    uum = np.reshape(uum.T, (-1, 176, 200))
    ...
    for i in range(label_profile.shape[1]):
        result = np.hstack((
            yy[:200, i].reshape((200, 1)),
            label_profile[:200, i].reshape((200, 1)),
            uum_profile[:200, i].reshape((200, 1))
        ))
